...
if ! ps -p "$first" >/dev/null; then
    echo "first process not found"
    exit 1
fi

if ! ps -p "$third" >/dev/null; then
    echo "third process not found"
    exit 1
fi
...
percent_limit=10
{
    while true; do
        ...
        if [[ $cpu_usage -gt 10 ]] && [[ $current_nice -lt 19 ]]; then
            echo "first process CPU usage: ${cpu_usage}% -> increasing nice by 1 from $current_nice..."
...
sleep 2
echo -e "\ncurrent CPU usage of process 1:"
